...
test('a user lists and creates boards', function () {
    ...
    $this->getJson('/api/v1/boards')->assertOk()->assertJsonFragment(['id' => $board->id]);

    $this->postJson('/api/v1/boards', ['workspace_id' => $workspace->id, 'name' => 'API board'])
        ->assertCreated()
...
test('outsiders cannot access a board', function () {
    ...
    $this->getJson("/api/v1/boards/{$board->id}")->assertForbidden();
});
...
test('admins update and delete a board, plain members cannot', function () {
    ...
    $this->patchJson("/api/v1/boards/{$board->id}", ['name' => 'Renommé'])
    ...
    $this->patchJson("/api/v1/boards/{$board->id}", ['name' => 'X'])->assertForbidden();
...
test('lists and cards can be fully managed', function () {
    ...
    $listId = $this->postJson("/api/v1/boards/{$board->id}/lists", ['name' => 'À faire'])->assertCreated()->json('data.id');
    ...
    expect(Card::find($cardId)->completed_at)->not->toBeNull();

    $list2 = $this->postJson("/api/v1/boards/{$board->id}/lists", ['name' => 'Fait'])->json('data.id');
...
test('labels can be managed', function () {
    ...
    $labelId = $this->postJson("/api/v1/boards/{$board->id}/labels", ['name' => 'Bug', 'color' => '#ef4444'])
