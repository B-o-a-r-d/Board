<div>
    <button
        ...
        title="Automations"
        aria-label="Automations du board"
    >
    ...
    @if ($showModal)
        <x-modal max-width="2xl" on-close="$wire.close()">
            <x-slot:header>
                <span class="flex items-center gap-2"><x-phosphor-robot class="h-5 w-5" /> Automations</span>
            </x-slot:header>
            ...
            <div class="space-y-4 p-5">
                    ...
                    @forelse ($automations as $automation)
                        <div wire:key="auto-{{ $automation->id }}" class="flex items-start justify-between gap-3 rounded-lg border border-neutral-200 p-3 dark:border-neutral-700 {{ $automation->is_active ? '' : 'opacity-60' }}">
                            ...
                            <div class="flex shrink-0 items-center gap-1">
                                <button type="button" wire:click="startEdit({{ $automation->id }})" title="Modifier" class="rounded p-1 text-neutral-400 hover:text-indigo-600 dark:hover:text-indigo-400"><x-phosphor-pencil-simple class="h-4 w-4" /></button>
                                <button type="button" wire:click="toggleActive({{ $automation->id }})" title="{{ $automation->is_active ? 'Désactiver' : 'Activer' }}" class="rounded p-1 text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200">
                                ...
                                <button type="button" wire:click="deleteAutomation({{ $automation->id }})" title="Supprimer" class="rounded p-1 text-neutral-400 hover:text-red-500"><x-phosphor-x class="h-4 w-4" /></button>
                            </div>
                        </div>
                    @empty
                        <p class="rounded-lg bg-neutral-50 px-3 py-4 text-center text-sm text-neutral-500 dark:bg-neutral-800/50 dark:text-neutral-400">Aucune automation. Créez-en une pour automatiser ce board.</p>
                    @endforelse
                </div>
                ...
                @if ($building)
                    ...
                    <form wire:submit="save" class="space-y-3 rounded-lg border border-indigo-200 bg-indigo-50/40 p-3 dark:border-indigo-500/30 dark:bg-indigo-500/5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">{{ $editingId ? 'Modifier l\'automation' : 'Nouvelle automation' }}</p>
                        <div>
                            <label class="mb-1 block text-xs font-medium uppercase tracking-wide text-neutral-500">Nom</label>
                            <input type="text" wire:model="name" placeholder="Ex : Terminer en entrant dans « Fait »" class="w-full rounded-lg border border-neutral-300 bg-white px-3 py-1.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/40 focus:outline-none dark:border-neutral-700 dark:bg-neutral-800">
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            ...
                            <div class="space-y-2 rounded-lg border border-neutral-200 p-2 dark:border-neutral-700">
                                <p class="text-xs font-semibold uppercase tracking-wide text-neutral-500">Déclencheur</p>
                            ...
                            {{-- Action --}}
                            <div class="space-y-2 rounded-lg border border-neutral-200 p-2 dark:border-neutral-700">
                                <p class="text-xs font-semibold uppercase tracking-wide text-neutral-500">Action</p>
                        ...
                        <div class="flex justify-end gap-2">
                            <button type="button" wire:click="$set('building', false)" class="rounded-lg px-3 py-1.5 text-sm text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800">Annuler</button>
                            <button type="submit" class="rounded-lg bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-indigo-500">{{ $editingId ? 'Enregistrer' : "Créer l'automation" }}</button>
                        </div>
                    </form>
                @else
                    <button type="button" wire:click="startCreate" class="flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-neutral-300 px-3 py-2 text-sm font-medium text-neutral-600 hover:border-indigo-400 hover:text-indigo-600 dark:border-neutral-700 dark:text-neutral-300 dark:hover:border-indigo-500 dark:hover:text-indigo-400">
                        <x-phosphor-plus class="h-4 w-4" /> Nouvelle automation
                    </button>
                @endif
            </div>
        </x-modal>
