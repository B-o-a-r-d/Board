...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden transition-opacity"
     ...
     wire:target="search, filterLabel, filterMember, filterDue, resetFilters, applyFilter, applyView, calendarStep, calendarToday">
    ...
    {{-- Desktop: month grid --}}
    <div class="hidden min-h-0 flex-1 flex-col overflow-auto rounded-xl border border-neutral-200 dark:border-neutral-800 sm:flex">
        <div class="grid grid-cols-7 border-b border-neutral-200 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900/60">
            @foreach ($calendar['weekDays'] as $weekDay)
                <div class="px-2 py-1.5 text-center text-xs font-medium uppercase tracking-wide text-neutral-400">{{ $weekDay }}</div>
            @endforeach
        </div>
        <div class="grid flex-1 auto-rows-fr grid-cols-7">
            @foreach ($calendar['weeks'] as $week)
                @foreach ($week as $day)
                    <div class="min-h-[6rem] border-b border-r border-neutral-100 p-1 dark:border-neutral-800/60 {{ $day['inMonth'] ? '' : 'bg-neutral-50/60 dark:bg-neutral-900/40' }}">
