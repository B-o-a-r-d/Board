...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden transition-opacity"
     ...
     wire:target="search, filterLabels, filterMembers, toggleLabel, toggleMember, toggleUnassigned, filterDue, resetFilters, applyFilter, applyView, sortTable, renameCard, setCardDue, moveCardToList">
                ...
                <tbody>
                    @foreach ($tableCards as $card)
                        <tr wire:key="row-{{ $card->id }}" class="border-b border-neutral-100 hover:bg-neutral-50 dark:border-neutral-800 dark:hover:bg-neutral-800/40">
                            ...
                            <td class="px-3 py-1.5">
                                <div class="flex -space-x-1.5">
                                    @foreach ($card->members as $member)
                                        <x-user-avatar :user="$member" size="xs" class="ring-2 ring-white dark:ring-neutral-900" />
                                    @endforeach
                                </div>
                            </td>
                            ...
                            {{-- Custom fields --}}
                            @foreach ($customFields as $field)
                                <td class="whitespace-nowrap px-3 py-1.5 text-neutral-600 dark:text-neutral-300">
                                    {{ $card->customFieldValues->firstWhere('custom_field_id', $field->id)?->value }}
                                </td>
                            @endforeach
