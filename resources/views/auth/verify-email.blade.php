<x-layouts.guest :title="__('Vérifier l\'e-mail')">
    <div class="mb-6">
        <h1 class="text-xl font-semibold">{{ __('Vérifiez votre e-mail') }}</h1>
        <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            {{ __('Un lien de vérification vient de vous être envoyé. Cliquez dessus pour activer votre compte.') }}
        </p>
    </div>

    @if (session('status') === 'verification-link-sent')
        <div class="mb-4 rounded-lg bg-green-50 px-4 py-3 text-sm text-green-700 dark:bg-green-500/10 dark:text-green-400">
            {{ __("Un nouveau lien de vérification vient d'être envoyé.") }}
        </div>
    @endif
