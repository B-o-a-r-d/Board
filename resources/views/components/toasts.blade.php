{{--
    Stacked toast notifications (adapted from docs/ui_elements/toasts.html).
    Include once in a layout. Trigger via window.toast() or a Livewire
    $dispatch('toast', ...) — see resources/js/toast.js.
--}}
<template x-teleport="body">
    <ul
        x-data="{
            ...
            deleteToastWithId(id) {
                for (let i = 0; i < this.toasts.length; i++) {
                    if (this.toasts[i].id === id) { this.toasts.splice(i, 1); break; }
                }
            },
            burnToast(id) {
                ...
                if (burnToastElement) {
                    ...
                    setTimeout(function () {
                        ...
                        setTimeout(function () { that.stackToasts(); }, 1);
                    }, 300);
                }
            },
            getToastWithId(id) {
            ...
            stackToasts() {
                ...
                let that = this;
                setTimeout(function () { that.calculateHeightOfToastsContainer(); }, 300);
            },
            positionToasts() {
                ...
                if (this.position.includes('bottom')) { middleToast.style.top = 'auto'; }
            },
            alignBottom(el1, el2) {
                let top1 = el1.offsetTop; let height1 = el1.offsetHeight; let height2 = el2.offsetHeight;
                el2.style.top = top1 + (height1 - height2) + 'px';
            },
            resetBottom() {
                for (let i = 0; i < this.toasts.length; i++) {
                    if (document.getElementById(this.toasts[i].id)) { document.getElementById(this.toasts[i].id).style.bottom = '0px'; }
                }
            },
            resetTop() {
                for (let i = 0; i < this.toasts.length; i++) {
                    if (document.getElementById(this.toasts[i].id)) { document.getElementById(this.toasts[i].id).style.top = '0px'; }
                }
            },
        ...
        @toast-show.window="
            if (event.detail.position) { position = event.detail.position; }
            toasts.unshift({
        ...
        x-init="
            $watch('toastsHovered', function (value) {
                if (layout == 'default') {
                    if (position.includes('bottom')) { resetBottom(); } else { resetTop(); }
                    if (value) { expanded = true; stackToasts(); }
                    else { expanded = false; stackToasts(); setTimeout(function () { stackToasts(); }, 10); }
                }
            });
        "
        class="fixed z-[99] block w-full group sm:max-w-xs"
        :class="{
    ...
    >
        <template x-for="(toast, index) in toasts" :key="toast.id">
            <li
                ...
                x-init="
                    if (position.includes('bottom')) { $el.firstElementChild.classList.add('opacity-0', 'translate-y-full'); }
                    ...
                    setTimeout(function () {
                        setTimeout(function () {
                            ...
                            setTimeout(function () { stackToasts(); }, 10);
                        }, 5);
                    }, 50);
                    setTimeout(function () {
                        setTimeout(function () {
            ...
            >
                <span
                    class="relative flex flex-col items-start w-full p-4 transition-all duration-300 ease-out bg-white border shadow-lg group border-neutral-100 sm:rounded-lg sm:max-w-xs dark:bg-neutral-800 dark:border-neutral-700"
                ...
                >
                        ...
                        <div class="relative">
                            <div
                                ...
                                :class="{
                                    ...
                                    'text-neutral-800 dark:text-neutral-100': toast.type == 'default'
                                }"
                            >
                    ...
                    <button
                        type="button"
                        @click="burnToast(toast.id)"
                        class="absolute right-0 p-1.5 mr-2.5 text-neutral-400 duration-100 ease-in-out rounded-full cursor-pointer hover:bg-neutral-100 hover:text-neutral-500 dark:hover:bg-neutral-700"
