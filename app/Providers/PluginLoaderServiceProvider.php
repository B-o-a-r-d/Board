 ...
 * never crashing the app boot. Registered after AppServiceProvider (which binds
 * the PluginRegistry the plugin providers register into).
 */
class PluginLoaderServiceProvider extends ServiceProvider
{
    ...
    {
        // Defer to "booted": the DB resolver is only wired up once providers have
        // booted, and registering a plugin provider here (post-boot) boots it
        // immediately — so it registers into the PluginRegistry right away.
        $this->app->booted(fn () => $this->loadInstalled());
    }
    ...
    /**
     * Map a package's PSR-4 namespaces and return its provider entries.
     *
    ...
    /**
     * The DB may be unavailable (before migrate, in some CLI contexts).
     */
