 ...
 * (`storage/app/plugins/<key>/`) and are booted by PluginLoaderServiceProvider.
 *
...
class PluginInstaller
{
    ...
    public function update(string $key, bool $confirmBreaking = false): PluginPackage
    {
        ...
        if ($this->isBreaking($package->version, $this->normalize($tag)) && ! $confirmBreaking) {
            throw new PluginInstallException(__('This is a breaking (major) update — confirm to proceed.'));
        }
    ...
    private function installTag(array $entry, string $tag): PluginPackage
    {
        ...
        if ($manifest === null) {
            throw new PluginInstallException(__('The release has no readable composer.json.'));
        }

        $constraint = $manifest['require']['board/plugin-sdk'] ?? null;

        if ($constraint !== null && ! Semver::satisfies($this->hostSdkVersion(), $constraint)) {
            throw new PluginInstallException(__('Incompatible with the host SDK (:host) — requires :need.', [
    ...
    private function downloadAndExtract(string $repo, string $tag, string $key): void
    {
        ...
        if (! $response->successful()) {
            throw new PluginInstallException(__('Could not download the release archive.'));
        }
        ...
        if ($zip->open($tmpZip) !== true) {
            @unlink($tmpZip);
            throw new PluginInstallException(__('The release archive is not a valid zip.'));
        }
        ...
        if ($inner === null || ! File::exists($inner.'/composer.json')) {
            File::deleteDirectory($extractDir);
            throw new PluginInstallException(__('The release archive is not a valid plugin package.'));
        }
    ...
    private function latestReleaseTag(string $repo): string
    {
        ...
        if (! $response->successful() || empty($response->json('tag_name'))) {
            throw new PluginInstallException(__('No published release found for this plugin.'));
        }
